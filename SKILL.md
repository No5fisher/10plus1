...
## Overview
Turn a listed-company question into an evidence-based investment decision. Start from the business, industry and competitive position; then test financial quality, management, valuation, risks and falsification conditions. The final output must distinguish facts, estimates and judgment.

For a full PDF/DOCX equity-research report, use the bundled template in `templates/report-template.md` (and the DOCX/PDF visual reference files if the runtime can read them).

## Core Principles
1. Stock = fractional ownership of a business; price is evaluated only after the business is understood.
...
4. Think probabilistically. Every thesis must include a bear case and explicit kill criteria.
5. Separate quality from valuation. A great company can be a poor investment at the wrong price.
...
8. Treat SBC/dilution as an owner-economics issue, not a cosmetic accounting adjustment. Read `references/SBC-RULES.md` whenever SBC may be material.
...
For time-sensitive metrics, state the period/date. For controversial claims, triangulate at least two independent sources when feasible. Clearly label:
...
- Company's position: challenger, enhancer, platform, incumbent, niche or commodity player
...
#### Mandatory SBC Gate
Before valuation, run the SBC materiality test in `references/SBC-RULES.md`.
...
Do not claim this is the only theoretically valid method; explain alternative company-specific treatment when used.
...
Reduce the entire thesis to 3-5 measurable variables. Each variable must have:
- Why it matters
...
### 8. Valuation
Do not rely on a single method. Use the methods appropriate to the company:
...
- Sum-of-the-parts when business segments require it

Always produce Bear / Base / Bull scenarios with explicit assumptions. Show expected return/range when enough data exists. If inputs are too uncertain, say valuation is low-confidence rather than invent precision.
...
Do not present the label as personalized financial advice. State that position sizing depends on the investor's objectives, diversification, tax situation and risk tolerance.
...
If generating a DOCX/PDF and the runtime supports document rendering, use the bundled visual references:
- `templates/投行风格公司投资研究报告模板.docx`
- `templates/投行风格公司投资研究报告模板.pdf`
